...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def _port_stats_reply_handler(self, ev):
	    ...
	    self.max_link()
	    self.check_links()
            #from pprint import pprint;pprint(paths_to_change)
	    ...
	    datapath = self.switchdic[switch]
            ...
            for path in paths_to_change:
                if path[2] == switch:	#if path is for current switch
                    ...
                    print(x,self.path[x],path[2],path)
    ...
    def rebuild_link(self, s1, s2, cost):
        ...
        randpath = random.randrange(1, len(alter_paths[s1][s2])+1)
        ...
        path_selected_random = alter_paths[s1][s2]['path'+str(randpath)]['path']
        for path in self.path:
	    if len(path):
	        for switch in range(1, len(path)-2):	#find paths that use this link
		    if path[switch] == s1 and path[switch+1] == s2:
		        paths_to_change.append([path[0], path[-1], s1, s2, path_selected_random]) #add path to paths_to_change
		        break
