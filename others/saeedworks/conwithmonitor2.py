...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        self.logger.info('---------------- ' '----------------- ----------------- ' '--------   -----    ------')
        flowlist = [flow for flow in body if flow.priority in [1,3]]
        from pprint import pprint
	pprint(flowlist)
	for stat in sorted([flow for flow in body if flow.priority in [1,3]], key=lambda flow: (flow.match['eth_src'],flow.match['eth_dst'])):
