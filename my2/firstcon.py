...
###temp
temp = [['h3','s3','s5','s6','h6'],['h6','s6','s5','s3','h3']]
...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        #self.logger.info('---------------- ' '----------------- ----------------- ' '--------   -----    ------')
        #flowlist = [flow for flow in body if flow.priority == 1]
        #for stat in sorted([flow for flow in body if flow.priority == 1], key=lambda flow: (flow.match['eth_src'],flow.match['eth_dst'])):
    	    #self.logger.info('%016x %8s %17s %8x %8d %8d', ev.msg.datapath.id, stat.match['eth_src'], stat.match['eth_dst'], stat.instructions[0].actions[0].port, stat.packet_count, stat.byte_count)
    ...
    def _port_stats_reply_handler(self, ev):
        ...
        for stat in sorted(body, key=attrgetter('port_no')):
    		if stat.port_no > 5:
        		continue
    		self.edgesnow[str('s'+str(ev.msg.datapath.id))][self.findNextHop('s'+str(ev.msg.datapath.id),'eth'+str(stat.port_no))] = (int(stat.rx_bytes) + int(stat.tx_bytes)) -  edges[str('s'+str(ev.msg.datapath.id))][self.findNextHop('s'+str(ev.msg.datapath.id),'eth'+str(stat.port_no))]
    		edges[str('s'+str(ev.msg.datapath.id))][self.findNextHop('s'+str(ev.msg.datapath.id),'eth'+str(stat.port_no))] = int(stat.rx_bytes) + int(stat.tx_bytes)
	...
	#pprint(self.edgesnow)
	#print(switchinpath)
	#pprint(self.path[datapath])
	#for i in self.edgesnow:
    	    #print(str(i)+' : '+str(self.edgesnow[i]))
    	#nexteth = 2
    	#actions = [parser.OFPActionOutput(nexteth)]
